...
export default function AddCocurricularEventDialog({ open, setOpen, }) {
    ...
    useEffect(() => {
        let filteredClass = classAndSection.filter((cc) => {
            const classMatch = sessionID == '' || sessionID == sessionID;
            return classMatch;
        });
            ...
            AxiosObj.request(config)
                .then((response) => {
                    if (response.data?.Success) {
                        toast.success(response.data.Data[0] || "Event has been added");
    ...
    const handleChange = async (event) => {
        ...
        for (const item of addStudentRow) {
            // console.log("s", item)
            // console.log("c", valueArray) 
            if (!valueArray.includes(item.classSectionIDA)) {
                // console.log("ss", item.classSectionIDA)
                // console.log("cs", valueArray)
    ...
    return (
        <div>
            <Dialog fullWidth={true} maxWidth='xl' open={open} onClose={handleClose} >
                <DialogTitle variant='h5' >Add Event</DialogTitle>
                <DialogContent >
                    <form  >
                        <Stack spacing={2} >
                            <Grid container justifyContent='space-around' alignItems={'center'} >
                                <Grid item lg={4} xs={12} flexDirection='column' paddingX={2}>
                                    ...
                                    <Input sx={{ display: 'block' }} placeholder='Event Name*' value={eventName} onChange={e => setEventName(e.target.value)} />
                                </Grid>
                                <Grid item lg={4} xs={12} flexDirection='column' paddingX={2}>
                                    <FormLabel>Event Description*</FormLabel>
                                    <Input sx={{ display: 'block' }} placeholder='Event Description*' value={eventDescription} onChange={e => setEventDescription(e.target.value)} />
                                </Grid>
                                <Grid item lg={4} xs={12} flexDirection='column' paddingX={2}>
                ...
                {loading ? <Loader /> : <Button variant='contained' color='warning' sx={{ padding: 1.5, paddingRight: 6, paddingLeft: 6, margin: 'auto', width: 'fit-content' }} onClick={handleAddEvent} >Add Event</Button>}
                <DialogActions>
